        ...
        <div class="product-detail-grid">
            ...
            <div class="product-gallery">
                <div class="pd-main-wrap">
                    <img src="<?= asset('images/uploads/' . $product['image']) ?>" alt="<?= e($product['name']) ?>" class="pd-main-img" id="mainImage">
                </div>
                <?php if (!empty($images)): ?>
                <div class="pd-thumbs">
                    <button class="pd-thumb is-active" type="button" onclick="changeImage(this.dataset.src, this)" data-src="<?= asset('images/uploads/' . $product['image']) ?>">
                        <img src="<?= asset('images/uploads/' . $product['image']) ?>" class="pd-thumb-img" alt="thumb">
                    </button>
                    <?php foreach ($images as $img): ?>
                        <button class="pd-thumb" type="button" onclick="changeImage(this.dataset.src, this)" data-src="<?= asset('images/uploads/' . $img) ?>">
            ...
            <div class="product-purchase-section glass-card">
                ...
                <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:20px;">
                    ...
                    <?php if ($product['label']): ?>
                        <span class="product-label" style="color: <?= e($product['label_text_color'] ?? '#ffffff') ?>; background: <?= e($product['label_bg_color'] ?? '#8b5cf6') ?>;">
                            <?= e($product['label']) ?>
                        </span>
                    <?php else: ?>
                        <div></div>
                ...
                <div style="margin-bottom:20px;">
                    <?php if ($product['discount_percent'] > 0): ?>
                        <div style="font-size:0.9rem;color:#64748b;text-decoration:line-through;margin-bottom:6px;">
                    ...
                    <div style="display:flex;align-items:center;gap:12px;">
                        <?php if (isset($_SESSION['currency']) && $_SESSION['currency'] === 'USD'): ?>
                            <div class="price" id="displayPrice" style="font-size:2rem;font-weight:800;color:#fbbf24;"><?= formatUSD($product['final_price_usd']) ?></div>
                        <?php else: ?>
                            <div class="price" id="displayPrice" style="font-size:2rem;font-weight:800;color:#fbbf24;"><?= formatVND($product['final_price_vnd']) ?></div>
                        <?php endif; ?>
                        <?php if ($product['discount_percent'] > 0): ?>
                            <span style="padding:6px 12px;background:#ef4444;color:#fff;border-radius:8px;font-size:0.7rem;font-weight:700;">-<?= $product['discount_percent'] ?>%</span>
                        <?php endif; ?>
                    </div>
                </div>
                <?php endif; ?>
                ...
                <!-- Stats -->
                <div style="display:flex;gap:20px;margin-bottom:20px;font-size:0.85rem;color:#94a3b8;padding-bottom:16px;border-bottom:1px solid rgba(139,92,246,0.15);">
                ...
                <div style="margin-bottom:20px;">
                    <div id="variantPriceDisplay" style="display:flex;align-items:center;gap:12px;">
                        <?php 
                        ...
                        <div id="originalPrice" style="font-size:0.9rem;color:#64748b;text-decoration:line-through;">
                            <?php if (isset($_SESSION['currency']) && $_SESSION['currency'] === 'USD'): ?>
                        ...
                        <?php if ($showOriginalPrice): ?>
                        <span id="discountBadge" style="padding:6px 12px;background:#ef4444;color:#fff;border-radius:8px;font-size:0.7rem;font-weight:700;">-<?= $firstVariant['discount_percent'] ?>%</span>
                        <?php endif; ?>
                    </div>
                </div>
                        ...
                        ?>
                            <label class="variant-label <?= $variant['stock'] <= 0 ? 'out-of-stock' : '' ?>">
                                <input 
                                    ...
                                    data-original-usd="<?= $variant['price_usd'] ?>"
                                    ...
                                    <div style="text-align:right;">
                                        <?php if (isset($variant['discount_percent']) && $variant['discount_percent'] > 0): ?>
                                            ...
                                            <div style="display:flex;align-items:center;justify-content:flex-end;gap:8px;margin-bottom:4px;">
                                                ...
                                                <span style="padding:2px 6px;background:#ef4444;color:#fff;border-radius:4px;font-size:0.65rem;font-weight:700;">-<?= $variant['discount_percent'] ?>%</span>
                                            </div>
                                        <?php endif; ?>
                                        ...
                                        <?php if ($variant['stock'] <= 0): ?>
                                            <span style="font-size:0.7rem;color:#ef4444;margin-top:2px;display:block;">(Hết hàng)</span>
                                        <?php elseif ($variant['stock'] < 10): ?>
                                            <span style="font-size:0.7rem;color:#64748b;margin-top:2px;display:block;">(Còn <?= $variant['stock'] ?>)</span>
                                        <?php endif; ?>
                                    </div>
                                </div>
                            </label>
                ...
                <div style="margin-bottom:18px;">
                    ...
                    <div style="display:flex;gap:8px;align-items:center;">
                        <button onclick="changeQty(-1)" class="btn btn-secondary" style="width:40px;height:40px;padding:0;font-size:1rem;">-</button>
                        <input type="number" id="quantity" value="1" min="1" max="<?= $maxOrder ?>" 
                               style="width:70px;height:40px;text-align:center;background:rgba(15,23,42,0.6);border:1px solid rgba(139,92,246,0.3);border-radius:8px;color:#f8fafc;font-size:0.95rem;font-weight:700;" readonly>
                        <button onclick="changeQty(1)" class="btn btn-secondary" style="width:40px;height:40px;padding:0;font-size:1rem;">+</button>
                    </div>
                </div>
                ...
                <div style="margin-bottom:1.5rem;padding:1rem;background:rgba(139,92,246,0.1);border:1px solid rgba(139,92,246,0.3);border-radius:12px;">
                    <label style="display:block;color:#f8fafc;font-weight:600;margin-bottom:0.5rem;font-size:0.95rem;">
                        ...
                        <?= e($product['customer_info_label'] ?? 'Thông tin cần thiết') ?> *
                    </label>
                    <textarea 
                           ...
                           style="background:rgba(15,23,42,0.8);border:1px solid rgba(139,92,246,0.3);color:#f8fafc;padding:0.75rem;border-radius:8px;font-size:0.9rem;resize:vertical;"
                           required></textarea>
                ...
                <?php if ($allOutOfStock): ?>
                    ...
                    <div style="background: rgba(15, 23, 42, 0.97); border: 1px solid rgba(71, 85, 105, 0.3); border-radius: 16px; padding: 32px; text-align: center;">
                        <i class="fas fa-box-open" style="font-size: 3rem; color: #475569; margin-bottom: 16px; display: block;"></i>
                        <div style="font-size: 1.1rem; font-weight: 700; color: #cbd5e1; text-transform: uppercase; letter-spacing: 1.5px; margin-bottom: 8px;">
                            Tạm hết hàng
                        </div>
                ...
                <?php else: ?>
                    ...
                    <div style="display:flex;flex-direction:column;gap:8px;">
                        <button onclick="buyNowDetail()" class="btn btn-primary" style="width:100%;padding:12px;font-size:0.95rem;font-weight:700;">
                             Mua Ngay
                        </button>
                        <button onclick="addToCartDetail()" class="btn btn-secondary" style="width:100%;padding:12px;font-size:0.95rem;">
                ...
                <div class="pd-tab-panel" id="tab-reviews">
                    <?php if ($reviewCount > 0): ?>
                        <?php foreach ($reviews as $review): ?>
                            <div class="review-item" style="padding:18px 0;border-bottom:1px solid rgba(139,92,246,0.15);transition:background 0.3s ease;">
                                <div style="display:flex;gap:15px;">
                                    <img class="review-avatar" src="<?= getUserAvatar($review) ?>" style="width:46px;height:46px;border-radius:50%;border:2px solid rgba(139,92,246,0.3);">
                                    <div class="review-content" style="flex:1;">
                                        <strong class="review-username" style="color:#f8fafc;font-weight:700;"><?= e($review['username']) ?></strong>
                                        <div class="review-stars" style="color:#fbbf24;margin:6px 0;">
                                            <?php for ($i = 1; $i <= 5; $i++): ?>
                                                <i class="<?= $i <= $review['rating'] ? 'fas' : 'far' ?> fa-star"></i>
                                            <?php endfor; ?>
                                        </div>
                                        <p class="review-comment" style="color:#94a3b8;line-height:1.6;margin:8px 0;font-size:14px;"><?= e($review['comment']) ?></p>
                                        <small class="review-time" style="color:#64748b;font-size:12px;"><?= timeAgo($review['created_at']) ?></small>
                                    </div>
                                </div>
                            </div>
                        <?php endforeach; ?>
...
function updateVariantPrice(radio) {
    ...
    const currency = '<?= isset($_SESSION['currency']) ? $_SESSION['currency'] : 'VND' ?>';
    ...
    const priceDisplay = document.getElementById('variantPriceDisplay') || document.getElementById('displayPrice').parentElement;
    
    let priceHTML = '';
    if (discount > 0) {
        // Show original price + discount badge + final price
        ...
        priceHTML = `
            <div style="font-size:0.9rem;color:#64748b;text-decoration:line-through;">${origPrice}</div>
            <div class="price" id="displayPrice" style="font-size:2rem;font-weight:800;color:#fbbf24;">${finalPrice}</div>
            <span style="padding:6px 12px;background:#ef4444;color:#fff;border-radius:8px;font-size:0.7rem;font-weight:700;">-${discount}%</span>
        `;
    } else {
        // Just show final price
        const finalPrice = currency === 'USD' ? '$' + priceUsd.toFixed(2) : new Intl.NumberFormat('vi-VN').format(priceVnd) + 'đ';
        priceHTML = `<div class="price" id="displayPrice" style="font-size:2rem;font-weight:800;color:#fbbf24;">${finalPrice}</div>`;
    }
...
function applyVoucher() {
    ...
    .then(data => {
        if (data.success) {
            ...
            // Update price display - simple and clean
            displayPrice.style.cssText = '';
            displayPrice.innerHTML = `
                <div>
                    <div style="margin-bottom:8px;">
                        <span style="font-size:1.8rem;color:#fbbf24;text-decoration:line-through;">${formattedOriginal}</span>
                        <span style="font-size:1.3rem;color:#ef4444;font-weight:800;margin-left:15px;">${voucherText}</span>
                    </div>
                    <div style="font-size:2.4rem;font-weight:900;color:#10b981;">${formattedFinal}</div>
                </div>
            `;
...
#paymentPopup.show, #successPopup.show {
    opacity: 1;
}
